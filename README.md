...
- **Performance** (15%): Core Web Vitals, loading speed
- **Accessibility** (11%): WCAG compliance, screen reader support
- **Code Quality** (11%): ESLint errors/warnings count
- **Build Time** (11%): Production build duration
- **Bundle Size** (11%): Final distribution size
...
```
TOTAL = (Performance × 0.15) + (Accessibility × 0.11) + (Code_Quality × 0.11) + 
        (Build_Time × 0.11) + (Bundle_Size × 0.11) + (Security × 0.10) + 
        (Best_Practices × 0.08) + (Package_Dependencies × 0.05) + (Lines_of_Code × 0.05) + 
        (File_Count × 0.05) + (SEO × 0.05) + (PWA × 0.05)
```
...
```
name             performance  accessibility  best-practices  code_quality  build_time  bundle_size  security  lines_of_code  file_count  seo   pwa   TOTAL
---------------  -----------  -------------  --------------  ------------  ----------  -----------  --------  -------------  ----------  ----  ----  -------
hot-cold-google  54%          10.00          9.60           8.00          1.7s        0.7MB        8.20      13144 lines    79 files    10.00 -     7.60
```
